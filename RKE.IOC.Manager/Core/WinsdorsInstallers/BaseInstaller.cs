    ...
    public abstract class BaseInstaller
    {
        ...
        protected IEnumerable<Type> GetTypesFromThisApplication()
        {
            ...
            var typesFromThisApplication =
                Directory.EnumerateFiles(path, "RKE*.dll", SearchOption.TopDirectoryOnly)
                .Concat(
                    Directory.EnumerateFiles(path, "RKE*.exe", SearchOption.TopDirectoryOnly)
                    )
                .Select(s =>
                {
                    try
        ...
        protected string GetInterfaceName(Type @interface)
        {
